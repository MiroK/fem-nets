...
@pytest.mark.parametrize('f', (df.Constant(1), df.Expression('2*x[0]*x[1]-3*x[1]', degree=2), ))
...
@pytest.mark.parametrize('f', (df.Constant(1), df.Expression('2*x[0]-3*x[1]+x[2]*x[1]', degree=2), ))
@pytest.mark.parametrize('get_points', (cell_centers, dof_coordinates, random_inside_points))
